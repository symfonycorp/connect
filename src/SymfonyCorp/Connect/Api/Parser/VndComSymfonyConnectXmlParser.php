...
use SymfonyCorp\Connect\Api\Entity\Member;
use SymfonyCorp\Connect\Api\Entity\Membership;
...
class VndComSymfonyConnectXmlParser implements ParserInterface
{
    ...
    protected function parseRoot(\DOMElement $element)
    {
        ...
        $projectsLink = $this->xpath->query('./atom:link[@rel="https://rels.connect.symfony.com/projects"]', $element);
        ...
        $clubsLink = $this->xpath->query('./atom:link[@rel="https://rels.connect.symfony.com/clubs"]', $element);
        if ($clubsLink->length) {
